...
export default function ProjectDetailPage() {
  const { id } = useParams()                          // ← Dynamic routing: useParams()
  const navigate = useNavigate()
  const { isFavorite, toggleFavorite } = useAppContext()

  const project = PROJECTS.find(p => p.id === Number(id))

  // Graceful 404 if project not found
  if (!project) {
    return (
      <main style={{ paddingTop: 120, minHeight: '100vh', textAlign: 'center', padding: '120px 24px' }}>
        <h1 style={{ fontFamily: 'var(--font-heading)', fontSize: 40, color: 'var(--color-text)', marginBottom: 16 }}>
          Project not found
        </h1>
        <Link to="/projects"
          style={{ color: 'var(--color-primary)', fontFamily: 'var(--font-body)', fontSize: 15 }}>
          ← Back to Projects
        </Link>
      </main>
    )
  ...
  return (
    <main style={{ paddingTop: 80, minHeight: '100vh' }}>
      ...
      {/* Hero banner */}
      <div style={{
        background: `linear-gradient(135deg, ${project.color}20 0%, var(--color-bg-alt) 100%)`,
        ...
        <div style={{ maxWidth: 860, margin: '0 auto' }}>
          <button onClick={() => navigate('/projects')}
            style={{ display: 'inline-flex', alignItems: 'center', gap: 6,
              ...
              fontSize: 14, cursor: 'pointer', marginBottom: 28, padding: 0, transition: 'color 0.2s' }}
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 20, flexWrap: 'wrap' }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap', marginBottom: 10 }}>
                <h1 style={{ fontFamily: 'var(--font-heading)', fontSize: 'clamp(26px,5vw,44px)',
            ...
            {/* Favorite button — reads/writes via Context */}
            <button onClick={() => toggleFavorite(project.id)}
              aria-label={fav ? 'Remove from favorites' : 'Star this project'}
              style={{
      ...
      <div style={{ maxWidth: 860, margin: '0 auto', padding: '48px 24px' }}>
        <div className="detail-grid" style={{ display: 'grid',
          gridTemplateColumns: '1fr 300px', gap: 40, alignItems: 'start' }}>

          {/* Left: Details + progress + buttons */}
          <div>
            ...
            <p style={{ fontFamily: 'var(--font-body)', fontSize: 15,
              color: 'var(--color-text)', lineHeight: 1.85, marginBottom: 36 }}>
              {project.details}
            </p>

            {/* Progress bar */}
            <h3 style={{ fontFamily: 'var(--font-heading)', fontSize: 15, fontWeight: 700,
              color: 'var(--color-text)', marginBottom: 10 }}>Completion</h3>
          ...
          <div>
            ...
            <div style={{ background: 'var(--color-surface)',
              border: '1px solid var(--color-border)', borderRadius: 'var(--radius-lg)', padding: 20, marginBottom: 20 }}>
            ...
            <div>
              <p style={{ fontFamily: 'var(--font-body)', fontSize: 11, fontWeight: 700,
                color: 'var(--color-text-muted)', letterSpacing: 2,
                textTransform: 'uppercase', marginBottom: 10 }}>Other Projects</p>
                ...
                return (
                  <Link key={p.id} to={`/projects/${p.id}`}
                    style={{ display: 'flex', alignItems: 'center', gap: 10,
                      ...
                      textDecoration: 'none', color: 'var(--color-text)', transition: 'color 0.2s' }}
                    onMouseEnter={e => e.currentTarget.style.color = p.color}
                    onMouseLeave={e => e.currentTarget.style.color = 'var(--color-text)'}>
                    <div style={{ width: 34, height: 34, borderRadius: 8, background: p.color,
                      color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center',
                      fontFamily: 'var(--font-heading)', fontSize: 11, fontWeight: 800, flexShrink: 0 }}>
                      {ini}
                    </div>
                    <div>
                      <div style={{ fontFamily: 'var(--font-body)', fontSize: 13, fontWeight: 600 }}>{p.title}</div>
